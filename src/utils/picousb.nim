...
#---- private functions -------
proc readLineInternal(self:PicoUsb,time:uint32=200)= #proc general reading of a usb string.
    while true: #until you find '\ 255' it runs ... 
...
proc setRedy(self:PicoUsb)= #proc to check if there is anything in the usb buffer. 
...
#----- pubblic functions --------
proc isRedy*(self:PicoUsb):bool= #procedure for checking the buffer status. 
    setRedy(self) #calls the procedure to set the variable.
    return self.setBool #return the value.

proc readLine*(self:PicoUsb,time:uint32=200):string= #proc for read the string in usb 
